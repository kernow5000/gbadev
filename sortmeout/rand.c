 ...
 * UNIRAN. Multiple (100) streams are supported with seeds spaced
  * 100,000 apart.
 ...
 * desired stream number. The header file rand.h must be included
  ...
  *    execute
  *     u = rand(stream);
  *    where rand() is a float function. The float variable u 
  *    will contain the next random number.
  * 
  * 2. To set the seed for stream "stream" to a desired value zset, 
  *    execute
  *     randst(zset, stream);
  *    where randst is a void function and zset must be a long set 
  *    to the desired seed, a number in [1, 2147483646].
  *    Default seeds for all 100 streams are given in the code. 
  * 
  * 3. To get the current (most recently used) integer in the
  *    sequence being generated for stream "stream" into the long
  *    variable zget, execute
 *     zget = rangt(stream);
