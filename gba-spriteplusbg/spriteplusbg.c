// sprite test with background this time
...
#include <kernow.h>     //my background data and pallete
...
void DrawBG() {
  ...
  for(loop = 0; loop < 256; loop++)
    BGPaletteMem[loop] = kernowPalette[loop];
  
  for(y = 0; y < 160; y++)
    for(x = 0; x < 120; x++)
      PlotPixel(x,y, kernowData[y*120+x]);
...
int main()
{
	...
	for(loop = 8192; loop < 8192 + 128; loop++)               //load sprite image data
       	{
       		OAMData[loop] = block1Data[loop - 8192];
       	}
