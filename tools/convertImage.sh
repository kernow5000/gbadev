...
then
    ...
    CMD="wine ../gfx2gba.exe $PCX_FILE $IMAGE_BIN -8 -v -s 2 -n -c $IMAGE_VAR -b -p $PAL_BIN"
    echo "command line: $CMD" >> output.txt
    echo "" >> output.txt
    wine ../gfx2gba.exe $PCX_FILE $IMAGE_BIN -8 -v -s 2 -n -c $IMAGE_VAR -b -p $PAL_BIN | more >> output.txt
